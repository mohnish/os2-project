...
public class FileSystem {
   ...
   public static final int nblocks = 100;
   public static final char[] fileName = "Seagate".toCharArray();
      ...
      while (true) {
         ...
         if (readInputValue == 1) {
            ...
            if (blockNum >= FileSystem.nblocks) {
               ...
               try {
                  byte[] buffer = null;
                  // accepting the data entered by the user
                  // trimming any white spaces in it
                  // and then converting the string into bytes
                  buffer = readData.readLine().trim().getBytes();
